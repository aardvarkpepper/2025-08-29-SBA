...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    res.json({ error: `An unspecified error has occured:  ${error}` });
    return;
    // Note to self:  Apparently 'return' is a convention for exiting programs early.  'return res.json' invokes res.json so sends a response, and return ends execution.  
  }
  if (isThereAnotherUserWithSameEmail) {
    res.status(400).json({ error: `400 Bad Request:  Another user with email ${email} already exists.` });
    return;
  }
